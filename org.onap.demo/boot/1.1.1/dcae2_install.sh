...
##echo "deb http://ppa.launchpad.net/openjdk-r/ppa/ubuntu $(lsb_release -c -s) main" >>  /etc/apt/sources.list.d/java.list
##echo "deb-src http://ppa.launchpad.net/openjdk-r/ppa/ubuntu $(lsb_release -c -s) main" >>  /etc/apt/sources.list.d/java.list
wget -O /etc/apt/sources.list https://github.com/peerless1230/onap_heat/mirrors/sources.list
apt-get update
...
add-apt-repository    "deb [arch=amd64] https://download.docker.com/linux/ubuntu \
   ...
   stable"
apt-get update
sudo apt-get install -y --fix-missing docker-ce
...
# upload DNS configuration with private key:/opt/app/config/key
echo "This is the option for FiberHome's environment, author: mlli5886"
echo "You can fix DNS problems by adding DNS config in setting of Openstack subnet"
# chmod 600 /opt/app/config/key
# scp $SSHOPTS -i /opt/app/config/key /opt/config/dns_ip_addr.txt ubuntu@FTP_SERVER:/home/ubuntu/share/onap.installation/
# chmod 777 /opt/app/config/key
