...
wget -O /root/id_rsa https://github.com/peerless1230/onap_heat/sshkey/onap_sample.pem
md5sum /root/id_rsa | awk '{ print $1 }' >/root/.mysqlpw
...
then
  if [! -f /etc/hadoop/conf/capacity-scheduler.xml ]
  then
    cp /usr/hdp/2.6.0.3-8/etc/hadoop/conf.empty/capacity-scheduler.xml /etc/hadoop/conf/capacity-scheduler.xml
  fi
...
then
	...
	(cd /bin; wget https://github.com/peerless1230/onap_heat/org.onap.dcaegen2.deployments/releases/cloud_init/check_cdap)
	chmod 755 /bin/check_cdap
	wget -qO- https://github.com/peerless1230/onap_heat/org.onap.dcaegen2.deployments/releases/cloud_init/instconsulagentub16.sh >/tmp/cinst.sh
